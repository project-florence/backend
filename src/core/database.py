    ...
    try:
        async with conn.cursor() as cur:  # tuple satirlar (row[0] erisimi icin)
            ...
            await cur.execute("""
                ...
                -- REFACTOR_PLAN.md Adim 3: gozlemlenebilirlik. Basarisiz bir
                -- cagrida token sayilari bilinmiyor (istek hic tamamlanmadi),
                -- bu yuzden var olan NOT NULL kisitini gevsetiyoruz --
                -- yoksa hata satiri INSERT'in kendisi basarisiz olurdu.
            ...
            await cur.execute("""
                CREATE TABLE IF NOT EXISTS exports (
                    ...
                    status TEXT NOT NULL DEFAULT 'queued'
                        CHECK (status IN ('queued','processing','ready','sent','failed')),
                    file_path TEXT,
                    token TEXT UNIQUE,
