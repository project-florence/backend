...
def _num(v: Any) -> float | None:
    """Coerce a value to a finite float, or return None when not possible."""
...
def peg_ratio(profile: dict) -> float | None:
    pe = _num(_g(profile, "valuation", "trailingPE"))
    g = _num(_g(profile, "financials", "earningsGrowth"))
    if pe and g:
        return round(pe / (g * 100), 2)
    return _num(_g(profile, "valuation", "pegRatio"))
...
def fcf_yield(profile: dict) -> float | None:
    fcf = _num(_g(profile, "financials", "freeCashflow"))
    mcap = _num(_g(profile, "market", "marketCap"))
    if fcf and mcap:
...
def price_to_sales(profile: dict) -> float | None:
    ps = _num(_g(profile, "valuation", "priceToSalesTrailing12Months"))
    if ps:
        return round(ps, 2)
    mcap = _num(_g(profile, "market", "marketCap"))
    rev = _num(_g(profile, "financials", "totalRevenue"))
    if mcap and rev:
...
def ev_to_ebitda(profile: dict) -> float | None:
    ev_ebitda = _num(_g(profile, "valuation", "enterpriseToEbitda"))
    if ev_ebitda:
        return round(ev_ebitda, 2)
    ev = _num(_g(profile, "valuation", "enterpriseValue"))
    ebitda = _num(_g(profile, "financials", "ebitda"))
    if ev and ebitda:
...
def book_value_per_share(profile: dict) -> float | None:
    bv = _num(_g(profile, "valuation", "bookValue"))
    if bv:
        return round(bv, 2)
    shares = _num(_g(profile, "trading", "sharesOutstanding"))
    equity = _num(_g(profile, "valuation", "enterpriseValue"))
...
def target_upside(profile: dict) -> float | None:
    price = _num(_g(profile, "market", "currentPrice"))
    target = _num(_g(profile, "valuation", "targetMeanPrice"))
    if price and target:
...
def volume_ratio(profile: dict) -> float | None:
    vol = _num(_g(profile, "market", "regularMarketVolume"))
    avg = _num(_g(profile, "trading", "averageVolume"))
    if vol and avg:
...
def fifty_two_week_position(profile: dict) -> float | None:
    price = _num(_g(profile, "market", "currentPrice"))
    high = _num(_g(profile, "market", "fiftyTwoWeekHigh"))
    low = _num(_g(profile, "market", "fiftyTwoWeekLow"))
...
def cash_to_debt(profile: dict) -> float | None:
    cash = _num(_g(profile, "balanceSheet", "totalCash"))
    debt = _num(_g(profile, "balanceSheet", "totalDebt"))
...
def quick_ratio(profile: dict) -> float | None:
    qr = _num(_g(profile, "balanceSheet", "quickRatio"))
    if qr:
        return round(qr, 2)
    return _num(_g(profile, "balanceSheet", "currentRatio"))


def sma_distance(profile: dict, ma: str = "fiftyDayAverage") -> float | None:
    price = _num(_g(profile, "market", "currentPrice"))
    avg = _num(_g(profile, "trading", ma))
    if price and avg:
