"""Unit tests for ``scripts/doctor.py`` -- REFACTOR_PLAN.md Adim 5.
...
1. An unconfigured purpose (``llm_settings`` has no row) is reported as WARN,
...
2. A missing ``FLORENCE_MASTER_KEY`` gets its own, unambiguous check
   (``llm_master_key``), separate from the per-purpose checks -- this was
...
def _resolved(purpose: str, *, provider_id: str = "openai", model: str = "gpt-5", api_key: str | None = "sk-test-real-secret-value") -> ResolvedLLM:
    return ResolvedLLM(
        purpose=purpose,
...
# ---------------------------------------------------------------------------
# 1) yapilandirilmamis amac -> WARN, FAIL degil
# ---------------------------------------------------------------------------


async def test_unconfigured_purpose_is_warn_not_fail(monkeypatch):
    async def _no_selection(purpose):
        return None

    monkeypatch.setattr(doctor, "get_selection", _no_selection)
    result = await doctor.check_llm_purpose("digest")
...
async def test_selected_but_unresolvable_is_fail(monkeypatch):
    ...
    async def _has_selection(purpose):
        return {"provider": "ghost-provider", "model": "x", "params": {}}

    async def _unresolvable(purpose):
        return Unconfigured(purpose=purpose, reason="saglayici 'ghost-provider' artik katalogda yok")

    monkeypatch.setattr(doctor, "get_selection", _has_selection)
    monkeypatch.setattr(doctor, "resolve_purpose", _unresolvable)
    result = await doctor.check_llm_purpose("report")
...
async def test_llm_purpose_detail_never_contains_raw_api_key(monkeypatch):
    secret = "sk-test-real-secret-value"

    async def _has_selection(purpose):
        return {"provider": "openai", "model": "gpt-5", "params": {}}

    async def _resolved_fn(purpose):
        return _resolved(purpose, api_key=secret)
    ...
    monkeypatch.setattr(doctor, "get_selection", _has_selection)
    monkeypatch.setattr(doctor, "resolve_purpose", _resolved_fn)
...
async def test_llm_purpose_model_not_in_roster_is_fail(monkeypatch):
    async def _has_selection(purpose):
        return {"provider": "openai", "model": "gpt-5", "params": {}}

    async def _resolved_fn(purpose):
        return _resolved(purpose, model="gpt-5")
...
async def test_llm_purpose_roster_unreachable_is_warn(monkeypatch):
    async def _has_selection(purpose):
        return {"provider": "openai", "model": "gpt-5", "params": {}}

    async def _resolved_fn(purpose):
        return _resolved(purpose)
...
async def test_llm_purpose_missing_key_for_key_requiring_provider_is_fail(monkeypatch):
    async def _has_selection(purpose):
        return {"provider": "openai", "model": "gpt-5", "params": {}}

    async def _resolved_fn(purpose):
        return _resolved(purpose, api_key=None)  # openai KEYLESS_PROVIDERS'ta degil
...
async def test_llm_purpose_last_call_error_is_fail(monkeypatch):
    async def _has_selection(purpose):
        return {"provider": "openai", "model": "gpt-5", "params": {}}

    async def _resolved_fn(purpose):
        return _resolved(purpose, provider_id="ollama-local", model="llama3", api_key=None)
